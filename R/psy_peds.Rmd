...
t.test2 <- function(m1, m2, s1, s2, n1, n2) {
  # t.test from sample statistics, ie using mean, standard deviation and count value of the samples
...
sex_by_period <- input_data %>% 
  ...
  mutate(percentage=round(num/num_total*100, 2)) %>%
  pivot_wider(names_from = sex, values_from = c(num, percentage))

sex_test <- chisq.test(sex_by_period[c("num_female", "num_male")])$p.value
...
summary_disorder_group_by_period <- input_data %>% 
  ...
  summarise(num = n_distinct(patient_num)) %>%
  group_by(period, add=TRUE) %>%
  mutate(percentage=round(num/sum( num )*100, 2)) %>%
  pivot_wider(id_cols = disorder_group, names_from = period, values_from = c(num, percentage)) %>% 
...
summary_disorder_group_by_period_test <- summary_disorder_group_by_period %>% 
  select(num_early_pandemic, num_late_pandemic) %>% 
  chisq.test()
