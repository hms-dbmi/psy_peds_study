...
summary_by_group_and_period <- input_data %>% 
  ...
  dplyr::mutate( when = ifelse( "early_pandemic" %in% period & "late_pandemic" %in% period, "before and after",
                                  ifelse( "early_pandemic" %in% period & ! "late_pandemic" %in% period, "before", "after"))) %>%
...
summary_by_group_and_period %>%
  ...
  labs(y = "Counts",
       x = "disorder group",
       title = "Patients with the same disorder group before and after the pandemic")+
  theme(legend.position = "bottom") + 
  scale_fill_grey()

ggsave("../output/disorder_before_after_pandemic.png", width = 18, height = 9, units="in")
```
