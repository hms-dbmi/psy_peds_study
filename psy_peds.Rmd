...
```{r}
### check that the ICD codes follow the regular expression [A-TUV-Z][0-9][0-9AB]\.?[0-9A-TV-Z]{0,4}
codesToReview <- obs_raw %>% dplyr::filter(concept_type == "DIAG-ICD10",
                                           ! grepl( "[A-TUV-Z][0-9][0-9AB]\\.?[0-9A-TV-Z]{0,4}", concept_code))
print(codesToReview)
...
print( paste0("There are ", length(mentalHealthCodes$concept_code), " ICD10 mental codes out of the total ",length(unique(icdCodes$concept_code)), " in the dataset"))
```
...
patient_count_barplot_period <- summary_by_group_and_period %>%
  dplyr::group_by( disorder_group, when ) %>%
  dplyr::summarise( count= n_distinct( patient_num ), siteid = site )

patient_count_per_week <- input_data[, c("patient_num", "weeks")] %>%
  unique() %>%
  group_by(weeks) %>%
  tally() %>%
  mutate(n = ifelse( n > obfuscation, n, obfuscation * 0.5), siteid = site )
...
save( patient_count_barplot_period, patient_count_per_week, patient_count_per_week_sex, patient_count_per_week_disorder_group, table1, length_hospitalisation_values, file = paste0("output/peds_psy_aggResults_", unique(as.character(demo_raw$siteid)), ".rda"))
