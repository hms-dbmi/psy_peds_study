...
```{r}
icdCodes <- read.csv("public-data/psychiatric_ICD-10_codes.csv", header = TRUE, colClasses = "character") %>% 
  rename(disorder_group = Mental_Health_Disorder_Group, 
...
We will also transform the observation dates, and break it by weeks, adding 2 additional columns: "weeks" and "year". Finally, we will dichotomize the study timeframe in three periods, based on cut-off dates, "before_pandemic", "early_pandemic" and "late_pandemic". 
...
patient_count_barplot_period <- input_data %>% 
  ...
  dplyr::mutate( 
    ...
    ) %>% factor(levels = c("Only Before Pandemic", "Before and During Pandemic", "Only During Pandemic"))
  ) %>%
  dplyr::ungroup() %>% 
...
patient_count_barplot_period %>%
  ...
  scale_fill_manual(values = cbPalette, name = "Period") +
  labs(y = "Unique patients count",
       x = "Disorder group",
       title = "Patients with at least one visits", 
...
patient_count_per_week %>%
  ...
  dplyr::mutate(period = ifelse( weeks < early_pandemic_start_date,
                                  "before_pandemic",
...
obs_by_day <- encounters_obs %>% 
  ...
  mutate(calendar_date = as.Date(calendar_date, format=dateFormat), 
         period = if_else(calendar_date < early_pandemic_start_date,
                          "Before Pandemic",
...
ggplot(obs_by_day, aes(x = calendar_date,
                       ...
                       color = period))  +
  geom_line() +
  geom_vline(xintercept = as.Date(early_pandemic_start_date),
             linetype = "dashed") +
...
patient_count_per_week_disorder_group %>%
  mutate(weeks = as.Date(weeks), 
         period = if_else(weeks < early_pandemic_start_date,
                          "Before Pandemic",
  ...
  labs(x = "Week",
       ...
       subtitle = "Grouped by mental health subcategories subcategories") + 
  ylim(0, NA) + 
...
table1 %>% 
  ...
  ungroup() %>%
  mutate(Category = "") %>%
  mutate(across(.cols = c(before_pandemic, early_pandemic, late_pandemic), .fns = replace_na, "0")) %>% 
